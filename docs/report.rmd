...
- `establish_connection` is used to open a TCP socket with the server, where the return of the function is the file descriptor fro the function. 
- `parse_message` is used to parse strings into `tmessage` messages to be send to the server. The implementation relies of regex to ensure proper message formatting.
